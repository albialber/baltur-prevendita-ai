...
# Etichette colonne e totale in base a listino/netto (valori correnti della UI)
if mostra_netto:
...
# Valori persistenti (per i rerun dopo la generazione)
...
def _normalizza_codice(c: str) -> str:
    """Normalizza il codice per confronti robusti (gestisce zeri iniziali e .0)."""
    s = str(c).strip()
    if s.endswith(".0"):
        s = s[:-2]
...
def trova_prodotto_da_codice(df: pd.DataFrame, code: str):
    """Ricerca robusta del record nel listino a partire dal codice."""
    s_code = str(code).strip()
    # match diretto
    mask = df["Codice"].astype(str) == s_code
    if mask.any():
        return df[mask].iloc[0]
    # match su codice normalizzato (senza zeri iniziali / .0)
...
if st.button("Genera preventivo", type="primary"):
    ...
    # Controllo speciale: blocco 4x MK 160 in cascata
    ...
    if invalid_mk_config:
        ...
        # Parte 1: DISTINTA MK
        # -----------------------
        # Calcola potenza totale MK per la logica INAIL (solo se configuratore MK usato, cascata)
        potenza_tot_mk = 0
        if cfg_input_mk is not None and hasattr(cfg_input_mk, "macro"):
            if cfg_input_mk.macro in ("INT_LINEA", "INT_ISOLA", "ESTERNO"):
                cald_dict = getattr(cfg_input_mk, "caldaie", {}) or {}
